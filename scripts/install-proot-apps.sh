...
# is on the minimal PATH that custom-cont-init.d scripts run with — so without
# this, `command -v proot-apps` fails and we silently skip every app. Harmless on
# older KasmVNC images that already have proot-apps on the system PATH.
